	...
	public class RiseAboveScoring
	{
		...
		public static void Main()
		{
			...
			board.Add(new Goal() { Position = 4, Height = 2, Color = "orange" });
			// END OF BOARD SETUP
		...
		{
			string color1 = ColorAtPosition(p_board, p_pos, 0);
			string color2 = ColorAtPosition(p_board, p_pos, 1);
			string color3 = ColorAtPosition(p_board, p_pos, 2);
		...
		public static int CalculateNumberOfCompletedRows(List<Goal> p_board)
		{
			...
			// Row 2 - horizontal 
			if(IsRowComplete(p_board, 3, 4, 5)) 
			{
				completedRows++;
			}

			// Row 3 - horizontal
			if(IsRowComplete(p_board, 6, 7, 8)) 
			{
				completedRows++;
			}

			// Row 4 - vertical
			if(IsRowComplete(p_board, 0, 3, 6)) 
			{
				completedRows++;
			}

			// Row 5 - vertical
			if(IsRowComplete(p_board, 1, 4, 7)) 
			{
				completedRows++;
			}
			
			// Row 6 - vertical
			if(IsRowComplete(p_board, 2, 5, 8)) 
			{
				completedRows++;
			}

			// Row 7 - diagonal
			if(IsRowComplete(p_board, 0, 4, 8)) 
			{
				completedRows++;
			}

			// Row 8 - diagonal
			if(IsRowComplete(p_board, 2, 4, 6)) 
			{
				completedRows++;
			}
		...
		public static int CalculateNumberOfCompletedStacks(List<Goal> p_board) 
		{
			...
			// Row 2 - horizontal
			completedStacks += CalculateCompletedStacksInRow(p_board, 3, 4, 5);
			// Row 3 - horizontal
			completedStacks += CalculateCompletedStacksInRow(p_board, 6, 7, 8);

			// Row 4 - vertical
			completedStacks += CalculateCompletedStacksInRow(p_board, 0, 3, 6);
			// Row 5 - vertical
			completedStacks += CalculateCompletedStacksInRow(p_board, 1, 4, 7);
			// Row 6 - vertical
			completedStacks += CalculateCompletedStacksInRow(p_board, 2, 5, 8);

			// Row 7 - diagonal
			completedStacks += CalculateCompletedStacksInRow(p_board, 0, 4, 8);
			// Row 8 - diagonal
			completedStacks += CalculateCompletedStacksInRow(p_board, 2, 4, 6);
